...
export class Renderer {
    ...
    project(p: Coordinate, cameraX: number, cameraY: number, cameraZ: number, cameraDepth: number, width: number, height: number, roadWidth: number) {
    ...
    renderBackground(offset: number) {
        this._backgrounds.forEach(background => {
            this._graphics.background(background.asset, this._graphics.width, this._graphics.height, background.parallaxMultiplier);
        });
    }

    renderRoad(camera: Camera, track: Track, position: Coordinate, playerZ: number) {
        const baseSegment = track.findSegment(position.z);
        const basePercent = (position.z % this.segmentLength) / this.segmentLength;
        ...
        for (let n = 0; n < this.drawDistance; n++) {
            ...
            const p1 = this.project(segment.p1, position.x * this.roadWidth - x, playerY + camera.height, position.z, camera.depth, this._graphics.width, this.height, this.roadWidth);
            const p2 = this.project(segment.p2, position.x * this.roadWidth - x - dx, playerY + camera.height, position.z, camera.depth, this._graphics.width, this.height, this.roadWidth);

            x += dx;
            dx += segment.curve;

            if (p1.camera.z <= camera.depth || p2.screen.y >= maxY || p2.screen.y >= p1.screen.y) {
                continue;
            }
