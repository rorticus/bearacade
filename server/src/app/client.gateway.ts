...
export class ClientGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    constructor(private _sessionService: SessionService, private _slackService: SlackService) {
        this._logger.log('Initialized client gateway');
    }

    @SubscribeMessage('connect')
    protected onConnect(client: ClientWithConnection, { sessionId }: { sessionId: string }) {
    ...
    async handleDisconnect(client: any): Promise<void> {
        const session = this._sessionService.findSessionByConnectionId(client.connectionId);
